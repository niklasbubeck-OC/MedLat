...
class Attention(nn.Module):
    ...
    def forward(
        ...
        out = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=context_mask, dropout_p=self.dropout)
...
class MaskGit(nn.Module):
    def __init__(
        ...
        self.transformer = MaskGitTransformer(num_tokens=num_tokens, dim=dim, depth=depth, seq_len=seq_len, dim_out = dim_out, t5_name = t5_name, text_embed_dim=text_embed_dim, self_cond = self_cond, dim_head = dim_head, heads = heads, ff_mult = ff_mult)
        self.self_cond = self.transformer.self_cond

        self.mask_id = self.transformer.mask_id
    ...
    def generate(
        ...
        batch_size = len(texts) if texts is not None else len(text_embeds)
        ...
        cond_ids = cond_ids.to(device) if exists(cond_ids) else None

        # text_embeds = default(text_embeds, self.transformer.encode_text(texts))
        ...
        if self.resize_image_for_cond_image:
            assert exists(cond_ids), 'conditioning image must be passed in to generate for super res maskgit'
