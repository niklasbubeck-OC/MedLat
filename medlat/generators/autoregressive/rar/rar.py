...
class RAR(nn.Module):
    def __init__(self, img_size=256, vae_stride=16, codebook_size=1024, width=768,
        ...
        self.embeddings = nn.Embedding(
            self.codebook_size + 1 + self.num_classes + 1, width)

        self.pos_embed = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(1, self.image_seq_len + 1024, width)), 0., 0.02)
        ...
        self.timesteps_embeddings = nn.init.trunc_normal_(
            nn.Parameter(torch.zeros(1, self.image_seq_len + 100, width)), 0., 0.02)
        self.adaln_before_head = FinalLayer(width, norm_layer=norm_layer)
        self.lm_head = nn.Linear(width,
                                 self.codebook_size, bias=True)

        self.none_condition_id = self.num_classes + self.codebook_size + 1
    ...
    def preprocess_condition(self, condition, cond_drop_prob=0.0):
        ...
        condition = condition + self.codebook_size + 1  # [0, 999] -> [codebook_size + 1, codebook_size + 999]
    ...
    def sample(self,
        ...
        if condition is None:
            condition = torch.ones((bsz,), device=device, dtype=torch.long) * self.none_condition_id
        else: 
            condition = self.preprocess_condition(
                condition, cond_drop_prob=cond_drop_prob)
        num_samples = bsz
        ...
        for step in tqdm(range(self.image_seq_len), disable=not progress):
            ...
            scale_pow = torch.ones((1), device=device) * cfg_pow
            scale_step = (1 - torch.cos(
                ((step / self.image_seq_len) ** scale_pow) * torch.pi)) * 1/2
            cfg_scale = (cfg - 1) * scale_step + 1
            if cfg != 0.0:
