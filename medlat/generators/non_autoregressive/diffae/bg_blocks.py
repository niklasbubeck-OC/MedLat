...
from .bg_choices import *
from .bg_config_base import BaseConfig
from torch import nn

from .bg_nn import (avg_pool_nd, conv_nd, linear, normalization,
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    """
    def forward(self, x, emb=None, cond=None, lateral=None, extra=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                if extra is not None:
                    x=th.cat((x,extra),dim=1)
...
class ResBlock(TimestepBlock):
    ...
    def __init__(self, 
        ...
        cond_emb_channels = self.cond_emb_channels or self.emb_channels
        ...
        # IN LAYERS
        #############################
        assert lateral_channels is None
        layers = [
        ...
        if use_condition:
            ...
            self.emb_layers = nn.Sequential(
                nn.SiLU(),
                linear(emb_channels, 2 * out_channels),
            )

            if two_cond:
                self.cond_emb_layers = nn.Sequential(
                    nn.SiLU(),
                    linear(cond_emb_channels, out_channels),
                )
            #############################
