...
def _resolve_ckpt_path(path: str) -> str:
    ...
    cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "medtok", "downloads")
    ...
    try:
        request = urllib.request.Request(path, headers={"User-Agent": "medtok/1.0"})
...
def init_from_ckpt(model, path: str, weights_only: bool = False) -> None:
    ...
    checkpoint is downloaded and cached under ~/.cache/medtok/downloads.
    """
