...
class ReconstructionLoss_Stage1(torch.nn.Module):
    ...
    def _forward_generator(self,
        ...
        print("target proxy shape", target_codes.shape)
        print("reconstructions shape", reconstructions.shape)
        reconstructions = reconstructions.contiguous()
        loss_fct = nn.CrossEntropyLoss(reduction="mean")
        batch_size = reconstructions.shape[0]
        print(reconstructions.view(batch_size, self.target_codebook_size, -1).shape)
        print(target_codes.view(batch_size, -1).shape)
