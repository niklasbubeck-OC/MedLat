...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        # Patch embedding (now grid-agnostic at forward)
        self.patch_embed = DynamicPatchEmbed(to_embed='conv', img_size=img_size, patch_size=patch_size, 
        ...
        if use_rope:
            ...
            self.encoder_pos_embed_learned = get_sincos_pos_embed(embed_dim=self.width, grid_size=self.grid_size, dims=dimension)
            self.encoder_pos_embed_learned = nn.Parameter(torch.tensor(self.encoder_pos_embed_learned), requires_grad=False)
            self.get_rope_tensor = None

    def mae_random_masking(self, x: Tensor, rope: Tensor | None, mask_ratio: float = -1):
        ...
        if mask_ratio == 0:
            ...
            rope_exp = None
            if rope is not None:
                rope_exp = rope.expand(bsz, -1, -1)
            return x, torch.zeros(bsz, seq_len, device=x.device), None, rope_exp
        ...
        if rope is not None:
            rope_exp = rope.expand(bsz, -1, -1)
            rope_visible = torch.gather(rope_exp, 1, ids_keep[..., None].repeat(1, 1, rope.shape[-1]))
    ...
    def forward(self, x: Tensor, mask_ratio: float = -1):
        ...
        if self.get_rope_tensor is not None:
            if self.dimension == 2:
                ...
                gh, gw = H // ph, W // pw
                rope = self.get_rope_tensor(self.head_dim, gh, gw).unsqueeze(0).to(x_tokens.device)
                grid = (gh, gw)
            else:
                D, H, W = x.shape[-3], x.shape[-2], x.shape[-1]
                pd, ph, pw = self.patch_embed.patch_size
                gd, gh, gw = D // pd, H // ph, W // pw
                rope = self.get_rope_tensor(self.head_dim, gd, gh, gw).unsqueeze(0).to(x_tokens.device)
                grid = (gd, gh, gw)

        x, _, ids_restore, rope_visible = self.mae_random_masking(x_tokens, rope=rope, mask_ratio=mask_ratio)

        x = self.ln_pre(x)
        if not self.use_rope:
            print("add learnable pos embed")
            x = x + self.encoder_pos_embed_learned

        for block in self.transformer:
            x = block(x, rope_visible if rope is not None else None, grid=grid)
        x = self.ln_post(x)
...
class Decoder(nn.Module):
    ...
    def forward(self, z_latents: Tensor, ids_restore: Tensor | None = None, target_img_size: tuple[int, ...] | None = None) -> Tensor:
        ...
        if not self.use_rope:
            print("add learnable pos embed")
            z = z + self.decoder_pos_embed_learned
        ...
        z = self.to_pixel(z, img_size=target_img_size)
...
@register_model("token.medtok.standard")
class MedTok(nn.Module): 
    ...
    def __init__(
        ...
        print(f"token channels: {token_channels}, gamma: {gamma}, finetune_decoder: {finetune_decoder}")
        ...
        self.gamma = gamma if not finetune_decoder else 0.0
        ...
        if finetune_decoder:
            logger.info(f"[DeTok] freezing all but decoder")
            self.freeze_everything_but_decoder()
    ...
    def forward(self, x: Tensor, return_posteriors: bool = False):
        ...
        decoded = self.decoder(z_latents, ids_restore=ids_restore, target_img_size=target_img_size)
